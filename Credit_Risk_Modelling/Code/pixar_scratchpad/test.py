...
def run():
    ...
    for i in range(data.shape[0]):
        ...
        for i in range(data.shape[0]):
            ...
            date_before_1Y = data['startTime_Date'][i][i] + one_year_rel
            #mask = (date_data['dateOpenedOrDisbursed'] < date_before_1Y) & (date_data['dateOpenedOrDisbursed'] != "NaT")
            mask = (pd.to_datetime(data['dateOpenedOrDisbursed'][i]) > pd.to_datetime(date_before_1Y)) & (pd.to_datetime(data['dateOpenedOrDisbursed'][i]) != "NaT")

            filtered_df = pd.DataFrame({"dateOpenedOrDisbursed":date_data['dateOpenedOrDisbursed'].loc[mask]}).sort_values('dateOpenedOrDisbursed')
            opened_last_list3Y = sorted(filtered_df['dateOpenedOrDisbursed'])
            # from itertools import combinations
            # combinatio= [",".join(map(str, comb)) for comb in combinations(opened_last_list, 2)]
            gap_month_relative = relativedelta(months=3)
            count_of_account =0
            # from itertools import combinations
            # datepair = [i for i in combinations(opened_last_list3Y, 2)]
            # for pair in datepair:
            #     diiff_days = (pd.to_datetime(pair[0]) - pd.to_datetime(pair[1])).days
            #     if (diiff_days>=-90)and (diiff_days<=90):
            #         dateOpened_gap3M_last1Y_CNT_val = dateOpened_gap3M_last1Y_CNT_val +1
            set_visited_date = set()

            #for date in opened_last_list3Y:
            for idx, date in enumerate(opened_last_list3Y):
                sublist =[]
                if date in set_visited_date:
                    if idx+1<len(opened_last_list3Y):
                        if opened_last_list3Y[idx+1] in set_visited_date:
                            continue
                    else:
                        continue
                date_after_3M = date + datetime.timedelta(days=90)
                mask=[]
                diiff_days =(pd.to_datetime(opened_last_list3Y)-pd.to_datetime(date_after_3M)).days

                from itertools import combinations
                c2 = [i for i in combinations(opened_last_list3Y, 2)]
                mask = ((diiff_days>=-90) & (diiff_days<0)) & (
                        pd.to_datetime(opened_last_list3Y) != "NaT")
                if sum(mask)>1:
                    if True in mask:
                        sub_filtered_df = pd.DataFrame({"dateOpenedOrDisbursed":filtered_df['dateOpenedOrDisbursed'].loc[mask]})
                        sublist = sorted(sub_filtered_df['dateOpenedOrDisbursed'])
                        if date in set_visited_date:
                            dateOpened_gap3M_last1Y_CNT_val = dateOpened_gap3M_last1Y_CNT_val + len(sublist)-1
                        else:
                            dateOpened_gap3M_last1Y_CNT_val = dateOpened_gap3M_last1Y_CNT_val+len(sublist)
                        if len(opened_last_list3Y)>1:
                            set_visited_date.update(sublist)
                            #opened_last_list3Y = sorted(list(set(opened_last_list3Y)-set(sublist)))
                        if len(opened_last_list3Y)==1:
                            pass
            print(filtered_df)
            for j in range(len(data['dateOpenedOrDisbursed'][i])):
